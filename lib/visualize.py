...
class Visualizer:
    ...
    def save_images(self, batch, pred_features, target_features, loss_notapplied_signal_vals, mode, step_index, sample=-1):
        ...
        def task_lines(task_name):
            lines = [
                ...
                '{:<8s}{:<8s} {:.1f}'.format('-', 'applied:', torch.mean((~loss_notapplied_signal_vals[task_name][sample]).float()).detach().cpu().numpy()),
            ]
            return lines
